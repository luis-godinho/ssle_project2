...
registry_rotations_total = Counter("registry_rotations_total", "Total port rotations", ["service"])
registry_heartbeats_total = Counter("registry_heartbeats_total", "Total heartbeats received")
...
def allocate_port(service_name, exclude_ports=None):
    ...
    with services_lock:
        used_ports = [s["port"] for s in services.values() if s.get("service_name") == service_name]
            ...
            "services": [
                {
                    ...
                    "healthy": info["healthy"],
                    "rotation_count": info["rotation_count"],
                    "seconds_since_heartbeat": int(current_time - info["last_heartbeat"]),
...
def rotate(service_name):
    ...
    with services_lock:
        ...
        logger.info(f"Rotation requested for {service_name}: {current_port} -> {new_port}")
        ...
        with services_lock:
            for name, info in services.items():
                if current_time - info["last_heartbeat"] > HEALTH_TIMEOUT:
                    if info["healthy"]:
                        logger.warning(f"Service {name} marked unhealthy (no heartbeat for {HEALTH_TIMEOUT}s)")
                        info["healthy"] = False
