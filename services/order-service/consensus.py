...
class BFTConsensus:
    def __init__(self, node_id: str, cluster_nodes: List[str]):
        ...
        node_name = node_id.split(":")[
            0
        ]  # Extract 'order-node-1' from 'order-node-1:8002'
    ...
    def _sign_vote(self, operation_id: str, vote: str) -> str:
        ...
        signature = hmac.new(
            self.auth_token.encode(), message.encode(), hashlib.sha256
        ).hexdigest()

        return signature

    def _verify_vote_signature(
        self, node_id: str, operation_id: str, vote: str, signature: str
    ) -> bool:
        """
        ...
        expected_signature = hmac.new(
            node_token.encode(), message.encode(), hashlib.sha256
        ).hexdigest()

        if signature != expected_signature:
            logger.error(
                f"BYZANTINE BEHAVIOR DETECTED: Invalid vote signature from {node_id}"
            )
        ...
        with self.operations_lock:
            if operation_id in self.pending_operations:
                ...
                if approved_count >= self.quorum_size:
                    ...
                    logger.info(
                        f"Operation {operation_id} COMMITTED ({approved_count}/{self.cluster_size} valid votes)"
                    )
                    return {
                        "success": True,
                    ...
                    logger.warning(
                        f"Operation {operation_id} REJECTED ({approved_count}/{self.cluster_size} valid votes, need {self.quorum_size})"
                    )
                    return {
                        "success": False,
                        "operation_id": operation_id,
    ...
    def _request_votes(
        self, operation_id: str, operation_type: str, operation_data: dict
    ) -> Dict[str, dict]:
        """
        ...
        for node_url in self.cluster_nodes:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Failed to get vote from {node_url}: {response.status_code}"
                    )
